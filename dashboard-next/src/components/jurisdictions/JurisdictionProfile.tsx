...
import { formatInstrumentType, formatDate, daysUntil } from '@/lib/utils';
import { jurisdictionDisplayName, unitNoun } from '@/lib/jurisdictions';
...
/**
 * Collision-safe bill predicate for a jurisdiction leaf. Keys on the (region, code) pair, never
 * `state` alone — US "DE" (Delaware) and "DE" (Germany) share a code but differ by region.
 *  - US:  region "US" + matching state code.
 *  - EU-wide (eu/eu):  every region "EU" act (member countries have their own profiles).
 *  - Foreign national law:  region == the country code.
 */
function billMatcher(region: string, code: string): (b: BillSummary) => boolean {
  if (region === 'US') return b => b.region === 'US' && b.state === code;
  if (region === 'EU' && code === 'EU') return b => b.region === 'EU';
  return b => b.region === region;
}

export function JurisdictionProfile({ region, code }: { region: string; code: string }) {
  region = region.toUpperCase();
  code = code.toUpperCase();
  const name = jurisdictionDisplayName(region, code);
  ...
  // Bill-explorer filters, scoped to this jurisdiction (the State select is hidden).
  const [filters, setFilters] = useState<BillFilterState>({ ...DEFAULT_FILTERS, state: isUS ? code : '' });
  ...
  function focusBills(partial: Partial<BillFilterState>) {
    setFilters({ ...DEFAULT_FILTERS, state: isUS ? code : '', ...partial });
    billsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }

  const match = useMemo(() => billMatcher(region, code), [region, code]);
  const jxBills = useMemo(() => bills.filter(match), [bills, match]);
  ...
  const stats = useMemo(() => {
    ...
    return { stages, inMotion, dead, total: jxBills.length };
  }, [jxBills]);
  ...
  // Deadlines are US-state-keyed; only meaningful for US jurisdictions.
  const upcomingDeadlines = useMemo(
    () =>
      isUS
        ? deadlines
            .filter(d => d.state === code)
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <Link href="/states/" className="text-sm text-green-accent hover:underline">&larr; All standings</Link>
      ...
      {/* Curated "beyond the bills" programs / incentives (US-keyed content layer) */}
      {programs.length > 0 && (
        <section className="space-y-3">
          <div>
      ...
      {/* Upcoming deadlines in this jurisdiction (US-keyed data) */}
      ...
      <section ref={billsRef} className="space-y-2 scroll-mt-6">
        ...
        {isLoading ? (
          ...
          <p className="text-text-secondary text-body">
            No tracked circularity legislation in {name} yet — {isUS ? 'wide-open territory.' : `a ${noun} we’re still building out.`}
          </p>
        )}
      </section>
    </div>
